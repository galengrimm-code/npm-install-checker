...
async function checkPackage(pkgName) {
  ...
  const [meta, downloads, audit] = await Promise.all([
    ...
    fetchJSON(
      `https://registry.npmjs.org/-/npm/v1/security/advisories?package=${encoded}&perPage=5`,
    ),
  ]);
  ...
  if (!meta) {
    ...
    if (similar) {
      results.push({
        check: "Typosquat",
        pass: false,
        detail: `Similar to popular package "${similar}"`,
      });
    }
    return results;
  }
  ...
  results.push(
    weeklyDl >= MIN_WEEKLY_DOWNLOADS
      ? { check: "Weekly downloads", pass: true, detail: `${weeklyDl.toLocaleString()}` }
      : {
          check: "Weekly downloads",
          pass: false,
          detail: `${weeklyDl.toLocaleString()} (threshold: ${MIN_WEEKLY_DOWNLOADS.toLocaleString()})`,
        },
  );
    ...
    results.push(
      ageDays >= MIN_AGE_DAYS
        ...
        : {
            check: "Package age",
            pass: false,
            detail: `First published ${ageDays} days ago (threshold: ${MIN_AGE_DAYS} days)`,
          },
    );
  }
    ...
    results.push(
      staleDays <= MAX_STALE_DAYS
        ...
        : {
            check: "Last publish",
            pass: false,
            detail: `${(staleDays / 365).toFixed(1)} years ago (threshold: 2 years)`,
          },
    );
  }
  ...
  results.push(
    maintainerCount >= MIN_MAINTAINERS
      ? { check: "Maintainers", pass: true, detail: `${maintainerCount}` }
      : {
          check: "Maintainers",
          pass: false,
          detail: `${maintainerCount} (threshold: ${MIN_MAINTAINERS})`,
        },
  );
  ...
  if (latestVersion) {
    ...
    results.push(
      depCount <= MAX_DEPENDENCIES
        ? { check: "Dependencies", pass: true, detail: `${depCount}` }
        : {
            check: "Dependencies",
            pass: false,
            detail: `${depCount} direct deps (threshold: ${MAX_DEPENDENCIES})`,
          },
    );
  }

  if (latestVersion) {
    const dangerousScripts = ["preinstall", "install", "postinstall"].filter(
      (s) => latestVersion.scripts?.[s],
    );
    results.push(
      dangerousScripts.length === 0
        ...
        : {
            ...
            pass: false,
            detail: `Has ${dangerousScripts.join(", ")} script(s)`,
          },
    );
  }
  ...
  if (!license) {
    ...
    results.push({
      check: "License",
      pass: true,
      detail: typeof license === "string" ? license : license.type || "Unknown",
    });
  }
